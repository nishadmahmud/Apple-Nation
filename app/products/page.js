...
export const metadata = {
  ...
  twitter: {
    card: "summary_large_image",
    title: "Shop Apple Products & Accessories | Apple Nation BD",
    description: "Explore the full Apple Nation BD collection with smart filters, search, and instant availability.",
  },
}
    ...
    const categoryPromises = categories.map(async (category) => {
      try {
        ...
        if (Array.isArray(firstPageResult?.data?.data)) {
          categoryProducts = [...firstPageResult.data.data]
          totalPages =
            firstPageResult?.data?.last_page ||
            firstPageResult?.data?.total_pages ||
            Math.ceil((firstPageResult?.data?.total || 0) / 20) ||
            1
        } else if (Array.isArray(firstPageResult?.data)) {
        ...
        if (totalPages > 1) {
          ...
          const remainingPages = remainingPageNumbers.map((page) =>
            fetchCategoryProducts(category.category_id, page, 20).catch(() => null),
          )
...
async function getCategoriesData() {
  const categoriesResult = await fetchCategories()
  return categoriesResult?.success && Array.isArray(categoriesResult?.data)
    ? categoriesResult.data.map((cat) => ({
        id: String(cat.category_id),
        name: cat.name,
        slug: cat.name.toLowerCase().replace(/\s+/g, "-"),
...
async function ProductsContent({ searchParams }) {
  ...
  const [categories, allProducts] = await Promise.all([getCategoriesData(), getAllProductsFromCategories()])
  ...
  if (categoryId !== "all") {
    try {
      ...
      if (totalPages > 1) {
        const remainingPageNumbers = Array.from({ length: totalPages - 1 }, (_, i) => i + 2)
        const remainingPages = remainingPageNumbers.map((page) =>
          fetchCategoryProducts(categoryId, page, 20).catch(() => null),
        )
